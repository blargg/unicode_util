...
impl Store {
    ...
    fn mk_config_dir() -> Result<PathBuf> {
        let mut path = dirs::config_dir().ok_or(StoreErr::MissingConfig)?;
    ...
    pub fn load_file() -> Result<Store> {
        ...
        if path.is_file() {
            let file_data = fs::read_to_string(path).map_err(|e| StoreErr::Io(e))?;
            toml::from_str(file_data.as_str()).map_err(|e| StoreErr::TomlDe(e))
        } else {
            Ok(Store::new())
        }
    }
    ...
    pub fn save_file(&self) -> Result<()> {
        let path = Store::store_path()?;
        let file_data = toml::to_string(self).map_err(|e| StoreErr::TomlSer(e))?;
        fs::write(path, file_data).map_err(|e| StoreErr::Io(e))
    }
}
